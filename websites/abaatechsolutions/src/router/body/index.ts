...
const GetBody = (payload:LoginProp)=>{
  return new Promise<APIResponseSchema>((resolve,reject)=>{  
  var st = `INSERT INTO newsletterSubscription (userEmail,newsSubscribed) VALUES(?,?)`;
  DBInstance(st,[payload.email,1]).then((res) => {
       if(res.status)
